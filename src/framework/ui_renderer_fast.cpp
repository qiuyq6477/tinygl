 ...
 * 返回值: 文本的总像素宽度。
 */
int UIRendererFast::textWidth(mu_Font font, const char *text, int len)
{
  int res = 0;
...
* 返回值: 文本的固定像素高度。
*/
int UIRendererFast::textHeight(mu_Font font) {
    return 18;
}

void UIRendererFast::init(mu_Context* ctx) {
    mu_init(ctx);
    ctx->text_width = UIRendererFast::textWidth;
    ctx->text_height = UIRendererFast::textHeight;
}

void UIRendererFast::processInput(mu_Context* ctx, const SDL_Event& e) {
    switch (e.type) {
...
void UIRendererFast::render(mu_Context* ctx, SoftRenderContext& renderCtx) {
    mu_Command* cmd = NULL;
